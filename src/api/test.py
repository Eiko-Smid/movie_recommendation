...
def _load_prev_production_state(model_name: str) -> Optional["Model_State"]:
    """
    Loads the current Production model from the MLflow Model Registry (if any)
    and returns its stored Model_State (so we can evaluate it fairly).
    """
    client = MlflowClient()
    ...
    # Our pyfunc stores Model_State inside python_model._state
    try:
        python_model = pyfunc_model._model_impl.python_model  # internal but widely used
        state = getattr(python_model, "_state", None)
        if isinstance(state, Model_State):
            return state
    except Exception:
        pass
    return None
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        registered = mlflow.register_model(
            ...
            name=model_name
                ...
                # _____________________________________________________________________
        # Champion/Challenger comparison + stage transition
        # _____________________________________________________________________
        model_name = os.getenv("MLFLOW_MODEL_NAME", "ALSRecommender")
        client = MlflowClient()

        # Evaluate the new model on the SAME eval set used during training
        # (here: your test_csr; keep K consistent with grid search)
        K_eval = 10
        new_metrics_eval = evaluate_als(
            model=best_model,
            train_coo=train_csr,        # we use train_csr for filtering seen items
            test_coo=test_csr,          # ground truth
            K=K_eval,
            num_threads=0,
            show_progress=False
        )
        new_map = float(new_metrics_eval.map_at_k)
        new_prec = float(new_metrics_eval.prec_at_k)

        mlflow.log_metrics({
            "new_precision_at_k_eval": new_prec,
            "new_map_at_k_eval": new_map
        })

        # Try to load the previous Production model and evaluate it on the same eval set
        prev_state = _load_prev_production_state(model_name)
        decision = "bootstrap_no_previous"

        if prev_state is None:
            # No previous Production model → make the freshly registered version the champion.
            ...
            prev_metrics_eval = evaluate_als(
            ...
            prev_map = float(prev_metrics_eval.map_at_k)
            prev_prec = float(prev_metrics_eval.prec_at_k)

            mlflow.log_metrics({
                "prev_precision_at_k_eval": prev_prec,
                "prev_map_at_k_eval": prev_map
            })

            # Decision rule (add tolerance if you like, e.g., eps = 1e-6)
            if new_map >= prev_map:
                client.transition_model_version_stage(
                    name=model_name,
                    version=registered.version,
                ...
                decision = f"promoted_new (new_map={new_map:.6f} >= prev_map={prev_map:.6f})"
            else:
                # Keep the existing Production; park the new one in Staging
                client.transition_model_version_stage(
                    name=model_name,
                    version=registered.version,
                    stage="Staging",
                )
                decision = f"kept_previous (new_map={new_map:.6f} < prev_map={prev_map:.6f})"

        # Helpful tags for the run
        mlflow.set_tags({
            "decision": decision,
            "eval_K": K_eval,
...
'''
        ...
        Compare the best metrics with the best metrics from grid search with the best metric from the 
        current best model. 

        Therefore load the current best model (if existing) and compute the metrics based on the current
        dataset. This will maybe change so we cant use the old metrics valeus from the model that we stored. 

        The new best model is the one with better map_@_k score.
        
        # Load old best model
        best_model_name = os.getenv("MLFLOW_MODEL_NAME", "ALSRecommender")
        best_model = _load_prev_production_state(best_model_name)
        decision = "bootstrap_no_previous"


        if best_model is None:
            # Make new model the prdocution model if no best model exist. 
            client.transition_model_version_stage(
                name=None, 
                version=None,
                stage="Production",
            ...
            # Evaluate old model on current data split
            best_model_metrics = evaluate_als(
                model=best_model,
                train_coo=train_csr,
                test_coo=test_csr,
                K=10,               # TODO: Change this to var given by endpoint request
                num_threads=0,
                show_progress=True
            )

            # Compare metrics -> choose new best model
            best_model_prec = best_model_metrics.prec_at_k
            best_model_map = best_model_metrics.map_at_k

            model_prec = metrics_ls[best_idx].prec_at_k
            model_map = metrics_ls[best_idx].map_at_k

            if model_map > best_model_map:
                client.transition_model_version_stage(
                    name=None,
                    version=None,
                    stage="Production",
                ...
                decision = f"promoted_new (model_map={model_map:.4f} >= best_model_map={best_model_map:.4f})"
